...
What it does
Maps the size and direction of the minority–majority difference across 53 countries
Ranks countries by the size of the difference, showing both group means side by side
Lets you search for a single country and see its religious composition and group averages
...
The World Values Survey microdata are not redistributed in this repository, in line with the WVS terms of use. To rebuild the aggregated data:

Download the WVS Wave 7 cross-national file from worldvaluessurvey.org and register for access.
Place the cleaned respondent-level file as data.rds in the project root, or point the readRDS() call in the preparation script at its location.
Knit prepare_dashboard_data.Rmd, which writes data/dashboard_data.rds.
Run the app:
r
shiny::runApp()
Dependencies
...
Religious minorities are somewhat more likely to leave the happiness item blank, so observed differences are likely conservative. Item non-response rates by group are reported in the app.
